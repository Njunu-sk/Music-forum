    ...
    <div v-show="showForm">
      ...
      >
        ...
        <div class="mb-3">
          ...
          />
          <ErrorMessage name="genre" class="text-red-500 text-xs italic" />
        </div>
        <button
          type="submit"
...
export default {
  ...
  methods: {
    async edit(values) {
      ...
      try {
        await songsCollection.doc(this.song.docId).set(values);
      } catch (error) {
    ...
    async deleteSong() {
      ...
      const songRef = storageRef.child(`songs/${this.song.docId}`);
